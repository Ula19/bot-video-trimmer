...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    # чистим возможный tmp_dir от предыдущей сессии обрезки
...
async def back_to_menu(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    # удаляем tmp_dir если остался от сессии обрезки, затем чистим state
    await cleanup_state(state)
...
async def trim_video_prompt(callback: CallbackQuery, state: FSMContext) -> None:
    """Нажатие на кнопку 'Обрезать видео' — запускаем FSM обрезки."""
    from bot.handlers.trim import TrimStates
    from bot.keyboards.inline import get_trim_cancel_keyboard
