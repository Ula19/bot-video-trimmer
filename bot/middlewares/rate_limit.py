"""Rate limiting — ограничение частоты запросов на обработку видео
...
def is_heavy_request(message: Message) -> bool:
    """Проверяет, является ли сообщение тяжёлой операцией (видеофайл).
    TODO: при необходимости расширить логику (document с video MIME и т.п.)
    """
    return message.video is not None or message.document is not None


class RateLimitMiddleware(BaseMiddleware):
    """Ограничивает частоту обработки — только для видеосообщений"""
    ...
    async def __call__(
        ...
        # ограничиваем только видеосообщения (тяжёлые операции)
